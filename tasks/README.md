...
- [ ] [100 — Can't create `---` under `---`, arrow-nav stuck between two `<hr>`](100-hr-create-and-arrow-nav.md) — 📋 **TODO** (bug, reported 2026-06-16): `<hr>` is a void block with no editable text, so the caret can't land between two adjacent rules — can't type a second `---` under one, and Up/Down can't move between them. Same class as the code-block/blockquote nav fixes (gap-paragraph + trailing invariant); real-VS-Code-only repro.
