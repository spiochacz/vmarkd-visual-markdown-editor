...
- [ ] [89 — Bump bundled ECharts 5.5.1 → 6.1.0](89-echarts-version-bump.md) — 📋 TODO (verify fidelity first): vendor 6.1.0 over Vditor's 5.5.1 (Lute/Mermaid `syncMermaid` pattern + `?v=` patch). **Major** bump (new default theme + option changes) → render verification required, unlike the same-major Mermaid bump. Do before 90.
- [ ] [90 — ECharts theme pairing (registerTheme + content-theme palette)](90-echarts-theme-pairing.md) — 📋 TODO (after 89): make `echarts` charts follow the content theme like Mermaid (task 86) — reuse the shared registry mapping + palette data, add ECharts-specific `paletteToEchartsTheme` + `registerTheme`/`init` (patch `chartRender`) + live re-render. Generalizes registry `mermaid` field → `palette`.
