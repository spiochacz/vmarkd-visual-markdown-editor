...
function deOvershoot(layout: Layout): void {
  ...
  // interiors, so a collapse that drops the route onto a container wall slips its guard — today it's only
    ...
    while (changed) {
      ...
      for (let i = 1; i + 3 <= p.length - 1; i++) {
        ...
        for (const corner of [[Dd[0], A[1]] as Pt, [A[0], C[1]] as Pt]) {
          ...
          if (hugsCont(A, corner) || hugsCont(corner, Dd)) continue
...
// Back-edge A* router (task 122). A "back-edge" climbs UP (its destination sits above its source); ELK
...
function rerouteBackEdges(layout: Layout): void {
  ...
  // edge-on-edge overlap. Guarding crossings alone let a reroute drop the route onto another edge's line
  // (microservices shipped with such an overlap the stale fixture quality-test missed) — task 123 #4.
  let best = countCrossings(layout)
  let bestCol = collinearOverlapCount(layout)
  for (const c of candidates) {
    const e = layout.edges[c.ei]
    const old = e.points
    e.points = c.pts as PlacedEdge['points']
    const x = countCrossings(layout)
    const col = collinearOverlapCount(layout)
    if (x <= best && col <= bestCol) {
      best = x
      bestCol = col
